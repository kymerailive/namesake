...
| Standing bands | **Five, named, reading `trust` and `warmth` and nothing else.** `RESENTED` (trust ≤ −20) · `WARY` (trust < 0) · `NEUTRAL` · `TRUSTED` (trust ≥ 28) · `WARM` (warmth ≥ 20). Exactly three consumers, and the brief's instruction to resist a fourth is held by there being one enum and one file: the trade price multiplier, whether one recipe is taught, and which dialogue pool is selected. Pool selection is *behaviourally identical* to session 09's four comparisons, which is what let session 11's Notice Board follow for free — the bands add distinctions the price and the board can see and the pools cannot. |
| Band thresholds | **Measured, not chosen, and each of the four boundaries has a different provenance.** `RESENTED` at −20 is where one witnessed killing puts a person, halved: a villager who watches you kill a neighbour lands at −40 and the person you killed at −54, while no single blow at any severity reaches −20 and twenty bare-handed punches to one person reach −19. `TRUSTED` **is** `Residency.TRUST_THRESHOLD` rather than a copy of its value, so §5's *"residency also grants the trusted price band"* is one constant instead of two that agree — and session 15 raised it from 20 to **28** in one edit that no test could see, which is exactly the property being described. `WARM` at 20 is session 09's `WARM_WARMTH`, moved unchanged **and deliberately not moved with it**: raising it to keep the pair level would need a peak warmth of 70 for the top discount to be permanent, against an observed maximum of 56–60, and the owner ruled that permanence at the close of session 13. `WARY` at 0 is session 09's hostile-pool boundary. |
| Standing threshold, and what the raise cost | **28, raised from 20 at session 15, and the two discounts stopped sitting on the same mark.** *Three in-game days to a discount is too fast* was the owner's ruling at the close of 13; the table it was picked from is `PersonalityDistributionTest.whatEachTrustThresholdCosts`, and the deciding column is the **median**, not the share — 24 leaves it at three days, so 24 moves only the tail and is a no-op on the complaint. **28 is the smallest candidate that takes the median to four.** Re-measured through the record layer rather than through the arithmetic: on one kindness a day the first resident of nine crosses on **day 15**, the third on **day 41** — this document said *day 28 at trust 20* — and **all nine never do** inside a hundred days, where at 20 they all did. The once-a-week player reached `TRUSTED` on one villager of nine at 20 and reaches it on **none** at 28, which is the raise doing precisely what it was asked to. **The cost is the middle rung, and it is measured rather than reasoned:** `Standing.of` tests warmth first, so eight points of gap means a saturating player skips `TRUSTED` — but only a *saturating* one. `StandingTest.theBandLadderIsWalked` walks all five player models and finds one kindness a day still goes `NEUTRAL → TRUSTED` nine times in nine; it is the twelve-gifts-a-day model that goes straight to `WARM`, seven of nine where it was two of nine. Somebody dumping a dozen gifts in an afternoon is buying warmth, and warmth is the discount that lapses. |
...
| Why not a standing split | **80% personal / 20% shared, server-configurable, was ruled from session 02 to session 15 and is struck at 15 rather than shipped.** Four reasons, and the first two were already written in this row before the session that had to decide it. **(1) It is a second route to a place gossip already reaches.** What a village collectively feels about you arrives at a villager who was not there through §4 step 7 — measured at 78% of a village within two in-game days — and it arrives *inside* the personal bond rather than beside it. This document has ruled against two answers to one question six times. **(2) Nobody can say what the shared term is computed from.** No settlement-scoped bond statistic exists anywhere in the codebase; the only settlement-wide aggregate is `Residency.verdict`, and it is a *count above a threshold*. For a village of nine where one resident holds 40 and eight hold 0, the mean is 4.4, the median 0, the count 1 and the maximum 40 — **four different mods, not four settings of one.** That is an unbuilt mechanic wearing a percentage, and "server-configurable" was hiding it: a number nobody has to choose is a decision nobody has to make. **(3) It would break session 12's one hard-won property silently.** `Standing.TRUSTED_TRUST` **is** `Residency.TRUST_THRESHOLD`, which is what makes §5's *"residency also grants the trusted price band"* true by construction. Blend the band's trust and leave residency's raw and the two diverge — and `StandingTest.residencyGrantsTheTrustedBand` **would still pass**, because its three residents all sit at exactly the threshold, so the settlement mean is the threshold. A property failing while its own guard stays green is the shape this project exists to refuse. **(4) The config exists now, which is what finally let the question be asked properly:** what would an operator set it to? Above 20 double-counts a signal already in the personal bond; 0 restores today's behaviour. **A knob whose only honest setting is zero is not a knob.** The cost of building it anyway is recorded rather than waved at: ten files and about twenty-three edits, seventeen of them collateral from `Dialogue.poolFor(Bond, boolean)` having no context to thread a settlement through — or a second entry point, which is two answers to one question a third time. And session 15 had already moved every band threshold once, 20 to 28, so shipping both would have made neither attributable. **What the split wanted already exists, at one place, as a boolean rather than a percentage: residency.** Three residents at the threshold changes what a villager who has *never met you* calls you, which is §5's whole moment. If a settlement-level term ever earns a second form, session 16's NPC-to-NPC bonds are where it would come from — with something to compute it from. |
...
change that at any mark. Trust does not decay, so it only ever climbs.
Residency is therefore earned by consistency rather than by intensity.

**The mark is 28, raised from 20 at session 15, and every figure under it was re-measured there.**
The owner ruled at the close of 13 that three in-game days to a discount is too fast; 24 leaves the
median at three days, so 28 is the smallest candidate that moves it. Through the record layer, at one
kindness a day over a hundred in-game days: **the first resident of nine crosses on day 15, the third
on day 41 — which is when this section's band is granted — and all nine never do.** This section said
*day 28* and that figure was taken at a threshold of 20. What changed at the far end is the part
worth reading: a player who turns up once a week reached this band on one villager of nine at 20 and
reaches it on **none** at 28.

**Derived, never persisted.** There is no residency flag and no grant ceremony: three residents at
the threshold, or one significant deed, computed from bonds and rings that are already on disk. A
...
anywhere near — three hungry people fed, against **forty-one** in-game days for the band at the
threshold session 15 set. Vanilla's own `wantsMoreFood` gates it in real play, so it is not free; but
the band the ruling is about is the slower path, **and the raise made the gap between the two routes
half again as wide**, which is a reason this question got more urgent rather than less. Whether that
is the right balance is still the owner's, and it is still open.
...
residents at the threshold is residency, one resident at it is the price. Move this
threshold and the price moves with it, which is what makes the claim a property of the code instead
of a coincidence somebody has to maintain. **Session 15 is when that was collected on** — 20 to 28
in one edit, and every villager in the world changed what they charge without a second constant
being touched or a single test going red. It also grants office eligibility at era ≥ 3, and —
critically — makes
...
**Nothing in `VillagerRenderer` is reusable, and that is a budget rather than a detail.** `VillagerModel`
is a `HierarchicalModel`; `HumanoidModel` is an `AgeableListModel`; they meet at `EntityModel` and
share nothing below it. So *"swap the renderer"* is a new renderer, a new model, **two**
`ModelLayerLocation`s and **two** `LayerDefinition`s — two because only `PlayerModel.createMesh`
takes a slim flag and `HumanoidModel.createMesh` has no slim variant at all.

| # | ruled | what it costs, and what it costs *us* |
|---|---|---|
| 1 | **Datapack-loadable means the variant set is *data*, and it is decided here rather than retrofitted.** The list of hairs, clothes and faces is a JSON manifest under `assets/namesake/`, read through the client's own `ResourceManager` on every resource reload. A texture path that is a Java constant is a retrofit; one resolved through the resource manager is not, and §9 has said *from v1* since before there was code. Adding a hair is one PNG and one line of JSON — **a merge, with no Java change** — which is the literal thing the line asks for. A resource pack overriding or extending it is the free dividend rather than the requirement. | **Selection is rendezvous hashing, not a modulus**, and that is the whole reason it is affordable. `seed % n` reshuffles every villager in the world the moment somebody appends a hair. Hashing the seed against each variant's own id and taking the winner moves about **one villager in n** to a new variant and leaves every other one exactly as they were. Six lines, and it is what makes "costs only a merge" true for the *player* as well as for the contributor. |
| 2 | **Profession legibility is re-earned in the eight clothing shapes. Our model does not implement `VillagerHeadModel`.** `VillagerProfessionLayer` is generically bounded on it, so the swap silently deletes vanilla's profession overlay — and session 13's headline was *you can tell who works by looking*. The tempting fix is to implement the interface and keep the layer. **It is refused on correctness, not on effort:** those overlay textures are authored against the *villager* mesh's UV map, and a humanoid mesh samples entirely different ones. It would compile, run, and draw nonsense. | **Two things are lost and both are named rather than discovered.** The **trade-level badge** (stone → diamond) is gone from the villager's belt; it is still drawn in the trade window, so it is lost *at a glance* rather than lost — **and it has no scheduled home**. The **biome type** overlay is gone too, and that one is replaced in kind rather than lost: a villager belongs to a culture in this mod, not to a biome, and the culture palette is the axis that actually exists. |
| 3 | **The colormap is sampled on the CPU, once per villager, and applied as a multiply tint.** §9 said *sample melanin × hemoglobin as UV*, which is a per-fragment texture lookup and needs a shader this mod does not have and will not grow in the last session of the slice. **This is a ruling made out loud rather than one made by writing the code.** | The PNG is still the single source of every skin tone, still sampled at melanin × hemoglobin, still editable by an artist and still overridable by a pack — which is why §9 chose a colormap over an HSV function in the first place, and all of it survives. **What is lost is stated: shadowed skin darkens without shifting hue.** Real skin goes redder in shadow; ours goes greyer. A greyscale base carries the luminance and one sampled colour carries the hue. |
| 4 | **Do not extend `HumanoidMobRenderer`.** Its constructor adds `CustomHeadLayer`, `ItemInHandLayer` **and `ElytraLayer`** — so the naive swap newly renders elytra on villagers, which is a thing this mod would have shipped and nobody would have looked for. Extend `MobRenderer` and add the three layers we want. | One of them is a dividend: vanilla drew the errand sack through `CrossedArmsItemLayer` because a villager's arms are crossed. Ours has real arms, so `ItemInHandLayer` puts the goods **in the hand**. `WORKPLAN.md` said the sack was *"invisible until session 15 swaps the renderer"* and that was wrong — it was always drawn; the real gate is `ShowTradesToPlayer`'s own preconditions. |
| 5 | **Three hooks, not one.** `VillagerRenderer` carries `scale()` = `0.9375 × getAgeScale()` and halves the shadow radius for babies. A humanoid model at 1.0 renders **every villager in the world visibly larger than vanilla**, and baby proportions move from the renderer's scale hook to `AgeableListModel.young`. | Budgeted as three, held by a test that reads the constant rather than by remembering. |
| 6 | **Layers, not composites.** Four passes over one model instance with a different texture and tint each — body, clothing, hair, face — which is exactly what `VillagerProfessionLayer` does. | Rejected: compositing a `DynamicTexture` per persona, which is MCA's answer. Four hundred personas at 64×64 RGBA is 6.5 MB of heap plus a GPU upload each plus **a cache to invalidate** — and this document has deleted a cache in four separate sessions for being a second copy of a truth. The layer pass is stateless. |
| 7 | **The client learns a villager's appearance from one clientbound packet of nine bytes, sent when a player starts tracking it.** The client knows *nothing* about a persona today: `PersonaLink` carries a bare UUID and is unsynced on both loaders. `appearanceSeed` would be free if the client had the persona id, and it does not; `cultureId` is assigned at placement and cannot be derived at all — `Cultures.at` needs the world seed. | **Priced before it was committed to, and it is cheap: `(entityId, appearanceSeed, cultureId)`, once per villager per player per tracking start, and nothing per tick.** Not an attachment sync, because that is two loader APIs that behave differently; this reuses the transport seam both loaders already have. **And the client cannot ask** — §2 rules only the server ever opens an interaction — so it is a push, and a villager whose persona has not been generated yet is re-sent the moment it is, from a branch the day plan already runs and only for villagers that have not been generated. |
